...
export default async function createProperty(req, res) {
  try { //An error can be caught; https://www.youtube.com/watch?v=P43DW3HUUH8&t=5957s 1:32:01
    ...
    if (user.role != "broker") {return res.json({popup: "Not a broker"});}
    const title = req.body.title;
    const propertyAlreadyThere = await Property.findOne({title});
    if (propertyAlreadyThere) {return res.json({popup: "Property already exists"});} //avoid having 2 titles, because a title is unique
    const property = new Property({
      ...
      await user.save();
      return res.json({popup: "Successfully created property"});
    }
